...
<div id="settings-modal-overlay">
  <div id="settings-modal">
    ...
    <div class="settings-section">
      ...
      <div id="bg-swatch-row" style="display:flex;gap:8px;flex-wrap:wrap;">
        <button class="bg-swatch-btn" data-value="default" title="Default" style="width:34px;height:34px;border-radius:50%;border:2px solid var(--border);cursor:pointer;background:#1a1a1a;"></button>
        <button class="bg-swatch-btn" data-value="midnight" title="Midnight Blue" style="width:34px;height:34px;border-radius:50%;border:2px solid var(--border);cursor:pointer;background:#0f172a;"></button>
        <button class="bg-swatch-btn" data-value="slate" title="Slate" style="width:34px;height:34px;border-radius:50%;border:2px solid var(--border);cursor:pointer;background:#111827;"></button>
        <button class="bg-swatch-btn" data-value="forest" title="Forest" style="width:34px;height:34px;border-radius:50%;border:2px solid var(--border);cursor:pointer;background:#0f1f16;"></button>
        <button class="bg-swatch-btn" data-value="plum" title="Plum" style="width:34px;height:34px;border-radius:50%;border:2px solid var(--border);cursor:pointer;background:#1f1425;"></button>
        <button class="bg-swatch-btn" data-value="rose" title="Rose" style="width:34px;height:34px;border-radius:50%;border:2px solid var(--border);cursor:pointer;background:#1f1417;"></button>
        <button class="bg-swatch-btn" data-value="charcoal" title="Charcoal" style="width:34px;height:34px;border-radius:50%;border:2px solid var(--border);cursor:pointer;background:#161616;"></button>
        <button class="bg-swatch-btn" data-value="ocean" title="Ocean" style="width:34px;height:34px;border-radius:50%;border:2px solid var(--border);cursor:pointer;background:#0b2027;"></button>
      </div>
    </div>
...
const BACKGROUND_PRESETS = {
  default:  { bg:'#1a1a1a', panel:'#2a2a2a', border:'#3a3a3a', accentDim:'#1a3a30', userBubble:'#2a2a2a', aiBubble:'#1a1a1a' },
  midnight: { bg:'#0f172a', panel:'#1e293b', border:'#334155', accentDim:'#164e63', userBubble:'#1e293b', aiBubble:'#0f172a' },
  slate:    { bg:'#111827', panel:'#1f2937', border:'#374151', accentDim:'#1f3a34', userBubble:'#1f2937', aiBubble:'#111827' },
  forest:   { bg:'#0f1f16', panel:'#16321f', border:'#2c4a35', accentDim:'#1f4a34', userBubble:'#16321f', aiBubble:'#0f1f16' },
  plum:     { bg:'#1f1425', panel:'#2b1c33', border:'#453151', accentDim:'#3a2350', userBubble:'#2b1c33', aiBubble:'#1f1425' },
  rose:     { bg:'#1f1417', panel:'#2b1a1e', border:'#4a2b31', accentDim:'#4a2330', userBubble:'#2b1a1e', aiBubble:'#1f1417' },
  charcoal: { bg:'#161616', panel:'#242424', border:'#383838', accentDim:'#2b2b2b', userBubble:'#242424', aiBubble:'#161616' },
  ocean:    { bg:'#0b2027', panel:'#123642', border:'#1f4e5c', accentDim:'#0e4a57', userBubble:'#123642', aiBubble:'#0b2027' },
};

let currentBackground = 'default';
function applyBackground(key) {
  const preset = BACKGROUND_PRESETS[key] || BACKGROUND_PRESETS.default;
  currentBackground = BACKGROUND_PRESETS[key] ? key : 'default';
  const root = document.documentElement.style;
  root.setProperty('--bg', preset.bg);
  root.setProperty('--panel', preset.panel);
  root.setProperty('--border', preset.border);
  root.setProperty('--accent-dim', preset.accentDim);
  root.setProperty('--user-bubble', preset.userBubble);
  root.setProperty('--ai-bubble', preset.aiBubble);
  document.querySelectorAll('.bg-swatch-btn').forEach(b => {
    b.style.outline = b.dataset.value === currentBackground ? '2px solid var(--accent)' : 'none';
...
document.querySelectorAll('.bg-swatch-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    applyBackground(btn.dataset.value);
    saveSettings();
  });
});
...
function classifyVoices(voices) {
  ...
  voices.forEach(v => {
    ...
    if (FEMALE_NAME_HINTS.some(h => name.includes(h))) female.push(v);
